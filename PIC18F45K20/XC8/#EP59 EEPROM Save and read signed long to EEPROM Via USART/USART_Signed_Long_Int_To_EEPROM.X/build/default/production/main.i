...
void main(void)
{
    ...
    while(1){
    USART_String_Transmit("\r\n Enter EEPROM Data Number : ");
    USART_String_Receive(ASCII_Conversion,11);
    Entered_Value = atol(ASCII_Conversion);
    writeLongIntoEEPROM(0x00, Entered_Value);
    number = readLongFromEEPROM(0x00);
    sprintf(ASCII_Conversion,"%s %li","\r\n EEPROM Value : ", number);
...
unsigned long writeLongIntoEEPROM(int address, long number)
{
  DATAEE_WriteByte(address, (number >> 24) & 0xFF);
  DATAEE_WriteByte(address + 1, (number >> 16) & 0xFF);
  DATAEE_WriteByte(address + 2, (number >> 8) & 0xFF);
  DATAEE_WriteByte(address + 3, number & 0xFF);
}

unsigned long readLongFromEEPROM(int address)
{
  return ((long)DATAEE_ReadByte(address) << 24) +
         ((long)DATAEE_ReadByte(address + 1) << 16) +
         ((long)DATAEE_ReadByte(address + 2) << 8) +
         (long)DATAEE_ReadByte(address + 3);
}
