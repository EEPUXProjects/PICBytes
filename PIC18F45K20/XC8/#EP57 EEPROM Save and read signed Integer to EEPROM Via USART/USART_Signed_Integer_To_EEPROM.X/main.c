/* Exercise : USART Signed Integer To EEPROM
 * Author : Usmaan H
 * Version : 1.0
 * Description : In this exercise we are using the serial terminal to save a signed 16 Bit integer value in two EEPROM Registers and 
...
void main(void)
{
    ...
    while(1){
    USART_String_Transmit("\r\n Enter EEPROM 6 Digit Number : ");// Output welcome string on UART.
    USART_String_Receive(ASCII_Conversion,6); // Enter a 6 digit number with zeros filling in unused integers. E.g value 23. Enter 000023.
    ...
    Entered_Value = Read_Int_From_EEPROM(ADDRESS_1);//Read EEPROM Addresses with stored integer and copy in number variable.
    sprintf(ASCII_Conversion,"%s %i","\r\n EEPROM Value : ", Entered_Value); //Convert Entered_Value variable into signed type string and concatenate with existing text.
