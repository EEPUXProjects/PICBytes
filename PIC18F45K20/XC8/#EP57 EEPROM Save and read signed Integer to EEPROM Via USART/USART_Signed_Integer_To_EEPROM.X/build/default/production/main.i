...
void main(void)
{
    ...
    while(1){
    USART_String_Transmit("\r\n Enter EEPROM 6 Digit Number : ");
    USART_String_Receive(ASCII_Conversion,6);
    Entered_Value = atoi(ASCII_Conversion);
    Write_Int_To_EEPROM(0x00,Entered_Value);
    Entered_Value = Read_Int_From_EEPROM(0x00);
    sprintf(ASCII_Conversion,"%s %i","\r\n EEPROM Value : ", Entered_Value);
...
unsigned int Write_Int_To_EEPROM(int address, unsigned int number)
{
    DATAEE_WriteByte(address, number >> 8);
    DATAEE_WriteByte(address + 1, number & 0xFF);
}

unsigned int Read_Int_From_EEPROM(int address)
{
  return (DATAEE_ReadByte(address) << 8)
