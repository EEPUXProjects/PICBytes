...
for (int Counter = 0x00 ; Counter <= 0x04; Counter++)
{
sprintf(ASCII_Conversion,"%s %u %s","\r\n Enter EEPROM Data Number ", Counter,": " );
USART_String_Transmit(ASCII_Conversion);
while(EUSART_is_rx_ready());
RAM = EUSART_Read()-0x30;
DATAEE_WriteByte(Counter,RAM) ;
}

for (int Counter = 0x00 ; Counter <= 0x04; Counter++)
{
Calculated_Value = DATAEE_ReadByte(Counter) + Calculated_Value ;
}

sprintf(ASCII_Conversion,"%s %u","\r\n EEPROM Calculated Value : ", Calculated_Value);
USART_String_Transmit(ASCII_Conversion);
